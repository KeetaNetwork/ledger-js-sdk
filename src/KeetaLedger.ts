...
export class KeetaLedger {
   ...
   */
	async getAccount(
		...
		return(await this.externalKeyPairAccount(index, algorithm, (data) =>
   ...
   */
	...
	private async externalKeyPairAccount(
		...
		let keyType: AccountKeyAlgorithm;
		if (algorithm === Algorithm.Secp256k1) {
			keyType = Account.AccountKeyAlgorithm.ECDSA_SECP256K1;
		} else if (algorithm === Algorithm.Ed25519) {
			keyType = Account.AccountKeyAlgorithm.ED25519;
		} else if (algorithm === Algorithm.Secp256r1) {
			keyType = Account.AccountKeyAlgorithm.ECDSA_SECP256R1;
		} else {
